...
public class EastPanel extends JPanel {
	...
	public void addFavoriates() {
		...
		System.out.println("Size of favouriates " + Singleton.getInstance().getFav().size());

		for (int i = 0; i < Singleton.getInstance().getFav().size(); i++) {

			if (checkifFavExists(Singleton.getInstance().getFav().get(i).getArea().getName()) == false) {
				JButton areas = new JButton(Singleton.getInstance().getFav().get(i).getArea().getName());
				System.out
						.println("This is the area name" + Singleton.getInstance().getFav().get(i).getArea().getName());
				favAreas.add(areas);
